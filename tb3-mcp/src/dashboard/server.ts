...
async function collect(s: Sources): Promise<SourceInputs> {
  const [
    deviceStatus, rigDirect, tracking, tracked, calibration, sun, capture, adsb, services, limits, taughtLimits, rezero,
  ] = await Promise.all([
    ...
    // Same get_taught_limits tool, its OTHER field (see TaughtLimits' doc,
    // state.ts) -- a second call, not a second leg of the SAME response,
    ...
    // get_rezero_status (Task 6 / I-C) -- re-polled every tick like
    // limits/taughtLimits above: a rezero_from_landmark/rezero_from_aircraft
    // call must clear the dashboard's re-zero banner on the very next poll,
    // not require a reload.
    tryResult(() => withTimeout(s.client.getRezeroStatus(), COLLECT_CALL_TIMEOUT_MS, "getRezeroStatus")),
  ]);
...
function emptySources(cfg: Config, cameraError: string | null): SourceInputs {
  return {
    ...
    rezero: NOT_POLLED_YET, // mergeState collapses this to rezero: null pre-first-poll
