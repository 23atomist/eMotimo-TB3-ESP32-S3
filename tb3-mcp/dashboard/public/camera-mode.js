...
// keeps three values on purpose, so that if MediaMTX misbehaves on the roof
// the operator can flip cameraSource back in config.json, restart, and get a
// known-good MJPEG picture instead of a dead panel. A source that is
// missing/degraded (not polled yet, or an older server build) must default
// to the historically-working MJPEG path, not the new WebRTC one.
