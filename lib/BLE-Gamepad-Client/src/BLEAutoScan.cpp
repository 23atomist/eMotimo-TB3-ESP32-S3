...
BLEAutoScan::BLEAutoScan(BLEControllerRegistry& controllerRegistry,
  ...
  // TB3 local patch: scan-callback attachment moved to initScanCallbacks();
  // NimBLEDevice::getScan() is not safe in global constructors on IDF 5.x.
}

void BLEAutoScan::initScanCallbacks() {
