...
- Produces:
  ...
  - `export interface CorrectorDeps { frames: FrameSource; detector: DetectorClient; postures: PostureHistory; predictPixel: (exposureMs: number) => PixelOffset | null; getOffset: () => AimOffset; applyOffset: (dPan: number, dTilt: number) => void; focalPx: () => number | null; frameSizePx: () => { widthPx: number; heightPx: number }; gain: () => number; readOnly: () => boolean; gateRadiusPx: () => number; minConf: () => number; log: (o: CorrectorOutcome, detail: Record<string, unknown>) => void }`
